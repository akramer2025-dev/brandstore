...
function findMatchingProducts(message: string, products: ProductInfo[]): ProductInfo[] {
  ...
  const scored = products.map(p => {
    ...
    if (productName.includes(query) || query.includes(productName)) score += 20
    if (productNameAr.includes(query) || query.includes(productNameAr)) score += 20
  ...
  // عرض كل المنتجات اللي عندها score أكبر من 0، مرتبة من الأعلى للأقل
  return scored
    .filter(s => s.score > 0)
    .sort((a, b) => b.score - a.score)
    .slice(0, 15) // زيادة العدد من 6 لـ 15 منتج
    .map(s => s.product)
}
...
- رقم التواصل الشخصي: 00966559902557
- لو حد عايز يسأل عن المطور أو يتعاقد معانا، ده رقم مستر أكرم المباشر
...
- إذا سأل العميل/العميلة عن منتج معين، ابحث في القائمة وقدم معلومات كاملة (السعر، المقاسات، الألوان، الوصف)
- المنتجات المطابقة هتظهر تلقائياً ككروت تحت ردك مع زرار "اطلب الآن" - متكتبش روابط
- لو سأل عن أكتر من منتج بنفس الاسم، اعرضهم كلهم - النظام هيجيب كل المنتجات المشابهة
- إذا سأل عن مقاس أو لون معين، تحقق من بيانات المنتج
- لو المنتج مش موجود، قول "للأسف مش متوفر حالياً بس تقدر تتواصل معانا على واتساب 01555512778 وهنوفرهولك إن شاء الله 💪"
